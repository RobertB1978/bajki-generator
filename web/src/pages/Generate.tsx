...
import { createStory, StoryRequest } from "../lib/api";
...
export default function Generate() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setOutput(md);
      const hist = JSON.parse(localStorage.getItem("stories") ?? "[]");
      hist.unshift({ ts: Date.now(), req: form, res });
      localStorage.setItem("stories", JSON.stringify(hist.slice(0, 50)));
    } catch (err: unknown) {
    ...
    <div className="max-w-3xl mx-auto p-6 space-y-6">
      ...
      <form className="grid gap-4" onSubmit={onSubmit}>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <label className="grid gap-1">
            <span>Długość</span>
            <select className="border p-2 rounded" value={form.length}
              onChange={e => setForm({ ...form, length: e.target.value as StoryRequest["length"] })}>
